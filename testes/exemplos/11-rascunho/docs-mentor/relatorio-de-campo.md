# Relatorio de campo · projeto · 29/08/26 14:00

Pacote **mentor-agent 0.2.2** · 0 tarefas concluidas

> Partes A e C sao geradas. A parte B e escrita, e item sem ancora e data **nao entra**.
> So metadado de processo: nada de codigo, requisito, nome de pessoa ou URL.

## A · Medicao

### A.1 Tarefas concluidas, por tipo

| Tipo | Qtd | % |
|---|---|---|

**Funcionalidade (RF+RN+RNF): sem dados.** Nenhuma tarefa concluida ainda: a proporcao so passa a medir alguma coisa a partir da primeira.

### A.2 Regras do pacote: escrita x comando

| Total | Viraram comando | Sao orientacao |
|---|---|---|
| 494 | 0 | 494 |

Regra sem comando nao e defeito: e orientacao declarada. O que importa e a proporcao mudar
entre uma versao e outra.

### A.3 Onde o pacote recusou

Nenhuma recusa registrada.

### A.4 Rotulos de gate

Nenhum gate registrado.

Tarefas com vermelho registrado antes do verde: **0 de 0**.

### A.5 Achados

Nenhum achado registrado.

### A.6 Divida tecnica e risco aceito

| Dividas abertas | Dividas pagas | Riscos ativos | Riscos vencidos |
|---|---|---|---|
| 0 | 0 | 0 | 0 |

### A.7 Fila

| Nascidas XG | Delas, fatiadas | Ordem fixada a mao | Validacoes pendentes |
|---|---|---|---|
| 0 | 0 | 0 | 0 |

**Ordem fixada a mao com frequencia significa que os criterios de ordenacao estao errados.**
Nesse caso quem muda e o pacote, nao quem usa.

### A.8 Tempo

| Dias parada antes de iniciar (mediana) | Dias em execucao (mediana) |
|---|---|
| - | - |

### A.9 Tetos de texto

Nenhum estouro.

### A.10 Melhorias anotadas durante o uso

1 anotacao(oes) por `mentor anotar --sobre pacote`.

- **29/08/26 14:00** · o doctor nao lembra de diagrama na descoberta

## B · Atrito (escrito a mao)

> Fonte: `docs-mentor/atrito-de-campo.md`, que o gerador nunca sobrescreve. Edite la; aqui e copia.

# Atrito de campo

> Escrito a mao. O `relatorio-de-campo` copia isto para dentro dele e **nunca sobrescreve** este
> arquivo. Item sem ancora e data nao entra: sem ancora nao da para conferir, e o que nao se
> confere vira reclamacao.

> **Tres ancoras:** `TASK-XXX-NNN` dentro de uma tarefa · `adocao` para instalar e inicializar ·
> `fase:<fase>` para o resto. Fases: ideia · descoberta · construcao · pre-lancamento · producao · manutencao.

### B.1 Regras que atrapalharam

- PREENCHER: <regra> · <ancora> · <data> · o que aconteceu · o que teria funcionado

### B.2 O que o pacote deixou de lembrar

- PREENCHER: <assunto> · <ancora> · <data> · quando isso deveria ter aparecido

### B.3 O que a IA teve que improvisar

- PREENCHER: <processo ausente> · <ancora> · <data>

## C · O que funcionou

- 494 regras no inventario, 0 com comando
- 0 de 0 tarefas encerradas sem nenhuma recusa
- 0 gates aprovados por execucao, nao por declaracao

> Analise honesta reconhece o que funciona, senao vira reescrita gratuita. Sem esta secao, todo
> relatorio vira lista de defeitos, e a leitura seguinte conclui que nada presta.
