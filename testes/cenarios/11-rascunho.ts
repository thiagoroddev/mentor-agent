...
export function rodar(): Cenario {
  ...
  const melhorias = ler(c, 'docs/melhorias-do-pacote.md')
  ...
  const rascunho = ler(c, 'docs/rascunhos/2026-08-29--avaliar-cache-de-sessao-no-login.md')
  ...
  mentor(limpo, 'init')
  const ctx = lerJson<Record<string, any>>(limpo, 'docs/contexto.json')
  ctx['estado'].fase = 'descoberta'
  escrever(limpo, 'docs/contexto.json', JSON.stringify(ctx, null, 2))
  dizQue(c, mentor(limpo, 'doctor'), 'sem nenhum rascunho',
    'fase inicial sem rascunho e cobrado pelo doctor, sem ninguem pedir')

  const ctx2 = lerJson<Record<string, any>>(c, 'docs/contexto.json')
  ctx2['estado'].fase = 'descoberta'
  escrever(c, 'docs/contexto.json', JSON.stringify(ctx2, null, 2))
  ...
  const limpo1 = ler(c, 'docs/relatorio-de-campo.md')
  ...
  confere(c, ler(c, 'docs/relatorio-de-campo.md').includes('o doctor nao lembra de diagrama'),
