# Reserva

<!-- Gerado por `npm run mentor`. Nao edite a mao: a proxima geracao sobrescreve. -->

> Lembretes sem compromisso. Nao entram no contexto: puxe com `mentor task puxar <ID>`.
> Requisito pendente nao precisa de tarefa aqui: `requisitos/pendentes.md` ja e o lembrete dele.

| ID | Titulo | Fatia | Tipo | Valor | Esforco H/IA | Origem |
|---|---|---|---|---|---|---|
| `TASK-RF-004` | Marcar entrega e desfazer | 3/5 de TASK-RF-001 | RF | critico | M/G | titulo-autossuficiente |
| `TASK-RF-005` | Status visual no mapa | 4/5 de TASK-RF-001 | RF | critico | P/M | titulo-autossuficiente |
| `TASK-RF-006` | Sumario reflete a execucao | 5/5 de TASK-RF-001 | RF | critico | P/M | titulo-autossuficiente |
